...
class TestSummaryExplanation:
    @patch("explanation.summary.client.chat.completions.create")
    def test_success_path(self, mock_create):
        ...
        result_top, result_warnings = summary_explanation(top_bullets, warning_bullets)

        assert result_top == "Rewritten prose here"
        assert result_warnings == warning_bullets
        
    @patch("explanation.summary.client.chat.completions.create")
    def test_empty_content_falls_back(self, mock_create):
        mock_response = MagicMock()
    ...
    @patch("explanation.summary.client.chat.completions.create")
    def test_api_exception_falls_back(self, mock_create):
